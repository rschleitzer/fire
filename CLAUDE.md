...
- String, token, date, reference, quantity search with modifiers (`:exact`, `:contains`, `:missing`)
- Search chaining (forward and reverse), `_include` and `_revinclude`
- Composite searches (code-value-concept, code-value-quantity)
...
│   └── structs.scm      # ✅ Rust models + search extractors
...
1. **Single SQL Migration** (`migrations/001_initial_schema.sql`) ✅
   - Generated by `migration.scm`
   - All active resources, current + history tables, indexes
2. **Rust Models + Search Extractors** (per resource: `src/models/{resource}.rs`) ✅
   - Generated by `structs.scm`
   - Resource structs (FromRow, Serialize, Deserialize)
   - History structs with operation tracking
   - VersionedResource trait impl
   - `inject_id_meta()` function
...
3. **Repository** (per resource: `src/repository/{resource}.rs`) ⏳
   - CRUD, search, history methods
4. **Handler** (per resource: `src/api/handlers/{resource}.rs`) ⏳
   - Axum routes, content negotiation

**Note on Search Extractors:**
The extractor generator in `structs.scm` (lines 96-534) handles all FHIR search parameter types:
- **Token**: Simple codes/booleans, CodeableConcepts (with system+code), Identifiers (system+value), ContactPoints
- **String**: HumanName (family, given, prefix, suffix, text), other text fields
- **Date**: Simple dates, choice types (DateTime/Period variants with proper field naming)
- **Reference**: Single references and collections
- **Quantity**: value, unit, system extraction
- **Deduplication**: Multiple search params on same element (e.g., `family`, `given`, `name` all on HumanName) generate extraction code once
...
- [x] 229/229 pyrtest tests passing
...
- [x] Generate 3 resources (SQL + Rust models + extractors)
- [x] Validate with pyrtest (229/229 passing)
- [x] Document DSSSL patterns
- [ ] Build repository generator
- [ ] Build handler generator
- [ ] Scale to more resources (after all generators complete)
