                            ...
                            if let Some((resource_type, chained_field)) = modifier_str.split_once('.') {
                                ...
                                if chained_field == "family" {
                                    ...
                                    if is_array {
                                        ...
                                        sql.push_str(&format!(
                                            "EXISTS (SELECT 1 FROM {} WHERE {}.id = SUBSTRING(observation.{}_reference FROM '[^/]+$') AND EXISTS (SELECT 1 FROM unnest({}.family_name) AS v WHERE v ILIKE ${}))",
                                    ...
                                    if is_array {
                                        ...
                                        sql.push_str(&format!(
                                            "EXISTS (SELECT 1 FROM {} WHERE {}.id = SUBSTRING(observation.{}_reference FROM '[^/]+$') AND EXISTS (SELECT 1 FROM unnest({}.given_name) AS v WHERE v ILIKE ${}))",
                                            target_table, target_table, ref_col, target_table, bind_idx
                                        ));
                                    }
                                    bind_values.push(format!("%{}%", param.value));
                                    ...
                                    if is_array {
                                        sql.push_str(&format!(
                                            "EXISTS (SELECT 1 FROM {} JOIN unnest(observation.{}_reference) AS ref_val ON {}.id = SUBSTRING(ref_val FROM '[^/]+$') WHERE {} ILIKE ${})",
                                            target_table, ref_col, target_table, target_table, bind_idx
                                        ));
                                    } else {
                                        sql.push_str(&format!(
                                            "EXISTS (SELECT 1 FROM {} WHERE {}.id = SUBSTRING(observation.{}_reference FROM '[^/]+$') AND {} ILIKE ${})",
                                            target_table, target_table, ref_col, target_table, bind_idx
                                        ));
                                    }
                                    bind_values.push(format!("%{}%", param.value));
